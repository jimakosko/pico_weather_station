...
private:
uint8_t PIN_RST; //20   //RESET PIN ACTIVE 0
uint8_t PIN_DC; // 21	   //DATA 1 COMMAND 0
uint8_t LCD_X=84;
uint8_t LCD_Y=48;
...
void writeComm(uint8_t *data,int size){
	lcd_command();
	link->spi_write(data,size);	
}	

public:	
nokia5110_lcd(spi_link *lnk,uint8_t rst=20,uint8_t dc=21){
	PIN_RST=rst;
	PIN_DC=dc;
	link=lnk;
...
void writeChar(uint8_t *data,int size){
	lcd_data();
	link->spi_write(data,size);	
}
...
void lcd_print_double(double num,int digits){
	if (num<0) 	lcd_print_char('-');
	num=fabs(num);
	int int_part;
	double dec_part;
...
//*******************************************************************
void lcd_print_str(std::string data){
